...
class Country < ApplicationRecord
  ...
  def set_or_update_country_info
    self.alpha2 = alpha2 || iso3166_country.alpha2
    self.region = region || iso3166_country.continent
    self.subregion = subregion || iso3166_country.subregion
    self.world_region = world_region || iso3166_country.world_region
    self.international_prefix = international_prefix || iso3166_country.international_prefix
    self.country_code = country_code || iso3166_country.country_code
  end

  before_save :set_or_update_currency_info
  def set_or_update_currency_info
    return if iso3166_country.currency.nil?
    self.currency_code = currency_code || iso3166_country.currency.iso_code
    self.currency_symbol = currency_symbol || iso3166_country.currency.symbol
    self.currency_name = currency_name || iso3166_country.currency.name
  end

  before_save :set_or_update_timezones_info
  def set_or_update_timezones_info
    new_timezones_offsets = []
    tz_info.zone_identifiers.each do |time_zone|
      new_timezones_offsets.push(Time.now.in_time_zone(time_zone).formatted_offset)
    end
    self.timezones = timezones || tz_info.zone_identifiers
    self.timezones_offsets = timezones_offsets || new_timezones_offsets
